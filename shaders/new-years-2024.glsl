...
vec2 transform3DPointToUV(vec3 point, vec3 normal, float camDist) {
    // Basic UV transformation
    vec2 uv = fract(vec2(point.x, point.y) * 0.1);

    // Apply distortion based on normal direction
    uv += normal.xy * 0.05; // Adjust the factor as needed
 ...
 { 
	...
	vec3 res = intersect(ro, rd);
	 if(res.x > 0.0){
           ...
           // Inside mainImage, after calculating 'p' and 'n'
            float camDist = length(ro - p);
            vec2 newUV = transform3DPointToUV(p, n, camDist);

            // Sample from iChannel0 using the new UV coordinates
            vec4 texColor = texture(iChannel0, newUV);
            col = texColor.rgb;
    } 
    ...
    if(res.x > 0.0){
        // Calculate the 3D point on the fractal surface
        //vec3 surfacePoint = ro + res.x * rd;

        
        //col = mix(texColor.rrr, vec3(luminance), 0.5);
    }
